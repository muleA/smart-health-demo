...
import { useGetEmployeeByEmployeeIdQuery, useUpdateEmployeeMutation } from "./employee.query";

const UpdateEmployeeForm = (props: { id?: string }) => {
  const [updateUser, { isLoading: isUpdating }] = useUpdateEmployeeMutation();
  const { data: employeeInfo, isLoading: isDetailsLoading } = useGetEmployeeByEmployeeIdQuery(props?.id as string);
...
const navigate=useNavigate()
  ...
  // Update form data when user details are fetched
   useEffect(() => {
    if (employeeInfo) {
      const { firstName, lastName, email, phone,gender,kebele,city,subCity,middleName,wereda} = employeeInfo;

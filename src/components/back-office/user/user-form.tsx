...
import {useGetUsersQuery, useAddNewUserMutation } from '../user/user-query';
import { useEffect } from 'react';
import { useParams } from 'react-router-dom';

const UserForm = (props: { id?: string, mode: "new" | "update" }) => {
  const [createUser, { isLoading: isCreating }] = useAddNewUserMutation();
  const [updateUser, { isLoading: isUpdating }] = useAddNewUserMutation();
  const { data: user, isLoading: isDetailsLoading }= useGetUsersQuery(0);
 ...
 */
 ...
 /*  useEffect(() => {
  ...
  return (
    <Card title={props.mode === 'new' ? 'Create User' : 'Update User'}>
      {isDetailsLoading ? (
